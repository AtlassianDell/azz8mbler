# z80_assembler.py - A complete two-pass Z80 assembler based on UM0080.pdf
...
# Global data structures for the assembler passes
symbol_table = {}
included_files = set()
macros = {} # Dictionary to store macro definitions

# Mapping of Z80 registers to their 3-bit and 2-bit opcode values
reg_map_8bit = {
...
# Special prefixes for IX/IY instructions
ix_iy_map = {'IX': 0xDD, 'IY': 0xFD}
ix_iy_reg_map = {'IXH': 0b100, 'IXL': 0b101, 'IYH': 0b100, 'IYL': 0b101}

# Mapping of conditional codes for jumps, calls, and returns
rel_jump_cond_map = {
...
def parse_expr(expr, location_counter, pass_num):
    """
    Parses a Z80 assembly expression. Handles decimals, hex ($), binary (%),
    character literals ('), and label lookups.
    """
    ...
    # Substitute the location counter symbol '#'
    ...
    # Use a regex to find and replace numbers and symbols
    resolved_expr = re.sub(r'\b[A-Z_][A-Z0-9_]*\b|\$[0-9A-F]+|%[01]+|\'[^\']\'', resolve_part, expr_with_lc)
    
    try:
        # Using a safe environment to evaluate the expression
...
def get_opcode(mnemonic, operands, location_counter, pass_num):
    """
    Generates the opcode bytes for a given Z80 mnemonic and its operands.
    This function is now a complete implementation of the Z80 instruction set.
    """
    
    operands = operands.strip()
    mnemonic = mnemonic.upper()
    
    # Handle `LD` instructions - one of the most complex groups
    if mnemonic == 'LD':
        ...
        if m_ld_ix_iy_r and src in reg_map_8bit:
            idx_reg, disp_str = m_ld_ix_iy_r.groups()
            disp = int(disp_str)
            if pass_num == 1: return [], 3
        ...
        # LD IXH/L, IXH/L and IYH/L, IYH/L (undocumented but common)
    ...
    # Handle `ADD`, `SUB`, `ADC`, `SBC`, `AND`, `XOR`, `OR`, `CP`
    if mnemonic in ['ADD', 'SUB', 'ADC', 'SBC', 'AND', 'XOR', 'OR', 'CP']:
        parts = [p.strip().upper() for p in operands.split(',')]
        
        # 8-bit operations on accumulator
        if len(parts) == 1:
            op = parts[0]
    ...
    # Handle Rotate and Shift Instructions (CB Prefix)
    ...
    # Handle Bit Manipulation Instructions (CB Prefix)
    ...
    # Handle `INI`, `INIR`, `IND`, `INDR`, `OUTI`, `OTIR`, `OUTD`, `OTDR`
    ...
    # Handle other miscellaneous opcodes
...
def main():
    ...
    try:
        ...
        print("Pass 1: Assembling to determine symbol locations...")
        symbol_table.clear()
        ...
        print("Pass 2: Assembling to generate machine code...")
        included_files.clear()
        try:
        ...
        with open(args.outfile, 'wb' if not args.plaintext else 'w') as f:
            if args.plaintext:
                ...
                f.write(hex_string)
                print(f"Assembly complete. Hex output written to '{args.outfile}'")
            else:
                f.write(bytes(machine_code))
                print(f"Assembly complete. Binary output written to '{args.outfile}'")
